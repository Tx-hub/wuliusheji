...
Page({
  ...
  onLoad: function () {
    ...
    qqmapsdk = new QQMapWX({
      key: 'ANFBZ-O3DCV-3NRPP-USEP4-NPGSJ-4VF6V'
    });
    wx.showLoading({
      title: '加载中'
    });
  ...
  selectedOk: function () {
    ...
    // console.log(this.data.centerData)
    var that=this
    console.log(prevPage.data.flag)
    if(prevPage.data.flag){
    prevPage.setData({
      beginaddr: this.data.info.city+this.data.info.district+this.data.info.title,
      begininfo:this.data.info,
    })
  }else{
    prevPage.setData({
      endaddr: this.data.info.city+this.data.info.district+this.data.info.title,
      endinfo:this.data.info,
